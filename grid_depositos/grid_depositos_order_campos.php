<?php
   ...
   session_start();
    $Ord_Cmp = new grid_depositos_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_depositos_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $_POST, $_GET;
   ...
   $tab_def_campos['id_depositos'] = "id_depositos";
   $tab_converte["id_depositos"]   = "id_depositos";
   $tab_ger_campos['deposito'] = "on";
   $tab_def_campos['deposito'] = "Deposito";
   $tab_converte["Deposito"]   = "deposito";
   $tab_ger_campos['direccion'] = "on";
   $tab_def_campos['direccion'] = "Direccion";
   $tab_converte["Direccion"]   = "direccion";
   $tab_ger_campos['cod_postal'] = "on";
   $tab_def_campos['cod_postal'] = "Cod_Postal";
   $tab_converte["Cod_Postal"]   = "cod_postal";
   $tab_ger_campos['localidad'] = "on";
   $tab_def_campos['localidad'] = "Localidad";
   $tab_converte["Localidad"]   = "localidad";
   $tab_ger_campos['provincia'] = "on";
   $tab_def_campos['provincia'] = "Provincia";
   $tab_converte["Provincia"]   = "provincia";
   $tab_ger_campos['responsable'] = "on";
   $tab_def_campos['responsable'] = "Responsable";
   $tab_converte["Responsable"]   = "responsable";
   ...
   {
       $tab_def_campos['sc_field_0'] = "`Teléfono`";
       $tab_converte["`Teléfono`"]   = "sc_field_0";
   }
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_depositos']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_depositos']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_depositos']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_depositos']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_depositos']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_depositos']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_depositos']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_depositos']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       $ordem = (substr($campo_sort, 0, 1) == "+") ? "asc" : "desc";
       $campo = substr($campo_sort, 1);
       $_SESSION['sc_session'][$sc_init]['grid_depositos']['ordem_select'][$campo] = $ordem;
   }
?>
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (isset($_SESSION['sc_session'][$sc_init]['grid_depositos']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
...
?>
       <OPTION value="<?php echo NM_encode_input($tab_def_campos[$NM_cada_field]); ?>" <?php echo $tmp_sel; ?>><?php echo $_SESSION['sc_session'][$sc_init]['grid_depositos']['labels'][$NM_cada_field]; ?></OPTION>
<?php
       }
   }
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_depositos']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
...
?>
             <OPTION value="<?php echo NM_encode_input($class . $NM_cada_field); ?>" selected><?php echo $class . $_SESSION['sc_session'][$sc_init]['grid_depositos']['labels'][$tab_converte[$NM_cada_field]]; ?></OPTION>
<?php
       }
   }
?>
