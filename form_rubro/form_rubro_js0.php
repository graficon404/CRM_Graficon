<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type=hidden name="id_rubro" value="<?php echo NM_encode_input($this->nmgp_dados_form['id_rubro']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_rubro']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
function NM_critica (form, campo, label, mask, tipo, intr, dec, min, max)
{
 ...
 NM_tab_crit[1] = new NM_critica('F1', 'descripcion_rubro', 'Descripcion Rubro', 'naomask', 'lista', 'cxab', 24, 'TUDO');
 NM_tab_crit[2] = new NM_critica('NM_final');
 NM_tab_crit_1 = new NM_critica(2);
 NM_tab_crit_1[1] = new NM_critica('F1', 'descripcion_rubro', 'Descripcion Rubro', 'naomask', 'lista', 'cxab', 24, 'TUDO');
