<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type=hidden name="id_forma_pago" value="<?php echo NM_encode_input($this->nmgp_dados_form['id_forma_pago']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_formas_pago']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
function NM_critica (form, campo, label, mask, tipo, intr, dec, min, max)
{
 ...
 NM_tab_crit[1] = new NM_critica('F1', 'descripcion_fp', 'Descripción', 'naomask', 'lista', 'cxab', 15, 'TUDO');
 NM_tab_crit[2] = new NM_critica('F1', 'incremento', 'Incremento', 'naomask', 'valor', 0, 6, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['incremento']['symbol_grp']); ?>', '<?php echo str_replace("'", "\'", $this->field_config['incremento']['symbol_dec']); ?>');
 NM_tab_crit[3] = new NM_critica('NM_final');
 NM_tab_crit_1 = new NM_critica(3);
 NM_tab_crit_1[1] = new NM_critica('F1', 'descripcion_fp', 'Descripción', 'naomask', 'lista', 'cxab', 15, 'TUDO');
 NM_tab_crit_1[2] = new NM_critica('F1', 'incremento', 'Incremento', 'naomask', 'valor', 0, 6, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['incremento']['symbol_grp']); ?>', '<?php echo str_replace("'", "\'", $this->field_config['incremento']['symbol_dec']); ?>');
