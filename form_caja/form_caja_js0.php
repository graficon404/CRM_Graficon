<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type=hidden name="id_caja" value="<?php echo NM_encode_input($this->nmgp_dados_form['id_caja']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_caja']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
function NM_critica (form, campo, label, mask, tipo, intr, dec, min, max)
{
 ...
 NM_tab_crit[1] = new NM_critica('F1', 'fecha', 'fecha', 'naomask', 'data', '<?php echo $this->field_config['fecha']['date_format_js']; ?>'<?php echo ", '', ''"; ?>, '<?php echo $this->field_config['fecha']['date_sep']; ?>');
 NM_tab_crit[2] = new NM_critica('F1', 'fecha_hora', 'fecha', 'naomask', 'hora', '<?php echo $this->field_config['fecha_hora']['date_format']; ?>', '<?php echo $this->field_config['fecha_hora']['time_sep']; ?>');
 NM_tab_crit[3] = new NM_critica('F1', 'detalle', 'detalle', 'naomask', 'lista', 'cxab', 30, 'TUDO');
 NM_tab_crit[4] = new NM_critica('F1', 'importe', 'importe', 'naomask', 'valor', 4, 2, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['importe']['symbol_grp']); ?>', '<?php echo str_replace("'", "\'", $this->field_config['importe']['symbol_dec']); ?>');
 NM_tab_crit[5] = new NM_critica('NM_final');
 NM_tab_crit_1 = new NM_critica(5);
 NM_tab_crit_1[1] = new NM_critica('F1', 'fecha', 'fecha', 'naomask', 'data', '<?php echo $this->field_config['fecha']['date_format_js']; ?>'<?php echo ", '', ''"; ?>, '<?php echo $this->field_config['fecha']['date_sep']; ?>');
 NM_tab_crit_1[2] = new NM_critica('F1', 'fecha_hora', 'fecha', 'naomask', 'hora', '<?php echo $this->field_config['fecha_hora']['date_format']; ?>', '<?php echo $this->field_config['fecha_hora']['time_sep']; ?>');
 NM_tab_crit_1[3] = new NM_critica('F1', 'detalle', 'detalle', 'naomask', 'lista', 'cxab', 30, 'TUDO');
 NM_tab_crit_1[4] = new NM_critica('F1', 'importe', 'importe', 'naomask', 'valor', 4, 2, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['importe']['symbol_grp']); ?>', '<?php echo str_replace("'", "\'", $this->field_config['importe']['symbol_dec']); ?>');
