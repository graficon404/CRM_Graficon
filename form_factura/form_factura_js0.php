<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type=hidden name="id_factura" value="<?php echo NM_encode_input($this->nmgp_dados_form['id_factura']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_factura']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
function NM_critica (form, campo, label, mask, tipo, intr, dec, min, max)
{
 ...
 NM_tab_crit[1] = new NM_critica('F1', 'fecha', 'fecha', 'naomask', 'data', '<?php echo $this->field_config['fecha']['date_format']; ?>'<?php echo ", '', ''"; ?>, '<?php echo $this->field_config['fecha']['date_sep']; ?>');
 NM_tab_crit[2] = new NM_critica('F1', 'remito_nro', 'remito_nro', 'naomask', 'numeroedt', 6, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['remito_nro']['symbol_grp']); ?>');
 NM_tab_crit[3] = new NM_critica('F1', 'pedido_nro', 'pedido_nro', 'naomask', 'numeroedt', 6, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['pedido_nro']['symbol_grp']); ?>');
 NM_tab_crit[4] = new NM_critica('F1', 'descuento', 'descuento', 'naomask', 'valor', 4, 2, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['descuento']['symbol_grp']); ?>', '<?php echo str_replace("'", "\'", $this->field_config['descuento']['symbol_dec']); ?>');
 NM_tab_crit[5] = new NM_critica('F1', 'observaciones', 'observaciones', 'naomask', 'lista', 'cxab', 32767, 'TUDO');
 NM_tab_crit[6] = new NM_critica('NM_final');
 NM_tab_crit_1 = new NM_critica(6);
 NM_tab_crit_1[1] = new NM_critica('F1', 'fecha', 'fecha', 'naomask', 'data', '<?php echo $this->field_config['fecha']['date_format']; ?>'<?php echo ", '', ''"; ?>, '<?php echo $this->field_config['fecha']['date_sep']; ?>');
 NM_tab_crit_1[2] = new NM_critica('F1', 'remito_nro', 'remito_nro', 'naomask', 'numeroedt', 6, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['remito_nro']['symbol_grp']); ?>');
 NM_tab_crit_1[3] = new NM_critica('F1', 'pedido_nro', 'pedido_nro', 'naomask', 'numeroedt', 6, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['pedido_nro']['symbol_grp']); ?>');
 NM_tab_crit_1[4] = new NM_critica('F1', 'descuento', 'descuento', 'naomask', 'valor', 4, 2, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['descuento']['symbol_grp']); ?>', '<?php echo str_replace("'", "\'", $this->field_config['descuento']['symbol_dec']); ?>');
 NM_tab_crit_1[5] = new NM_critica('F1', 'observaciones', 'observaciones', 'naomask', 'lista', 'cxab', 32767, 'TUDO');
