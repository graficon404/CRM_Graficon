<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type=hidden name="id_condicion_vta" value="<?php echo NM_encode_input($this->nmgp_dados_form['id_condicion_vta']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_condicion_venta']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
function NM_critica (form, campo, label, mask, tipo, intr, dec, min, max)
{
 ...
 NM_tab_crit[1] = new NM_critica('F1', 'decripcion_vta', 'Decripcion Vta', 'naomask', 'lista', 'cxab', 15, 'TUDO');
 NM_tab_crit[2] = new NM_critica('F1', 'entrega', 'Entrega', 'naomask', 'valor', 4, 2, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['entrega']['symbol_grp']); ?>', '<?php echo str_replace("'", "\'", $this->field_config['entrega']['symbol_dec']); ?>');
 NM_tab_crit[3] = new NM_critica('F1', 'vecimiento', 'Vecimiento', 'naomask', 'numeroedt', 2, -0, 99, '<?php echo str_replace("'", "\'", $this->field_config['vecimiento']['symbol_grp']); ?>');
 NM_tab_crit[4] = new NM_critica('NM_final');
 NM_tab_crit_1 = new NM_critica(4);
 NM_tab_crit_1[1] = new NM_critica('F1', 'decripcion_vta', 'Decripcion Vta', 'naomask', 'lista', 'cxab', 15, 'TUDO');
 NM_tab_crit_1[2] = new NM_critica('F1', 'entrega', 'Entrega', 'naomask', 'valor', 4, 2, -0, 999999, '<?php echo str_replace("'", "\'", $this->field_config['entrega']['symbol_grp']); ?>', '<?php echo str_replace("'", "\'", $this->field_config['entrega']['symbol_dec']); ?>');
 NM_tab_crit_1[3] = new NM_critica('F1', 'vecimiento', 'Vecimiento', 'naomask', 'numeroedt', 2, -0, 99, '<?php echo str_replace("'", "\'", $this->field_config['vecimiento']['symbol_grp']); ?>');
